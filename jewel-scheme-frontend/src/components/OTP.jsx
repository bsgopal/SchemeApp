  ...
  function OTP() {
    ...
    const handleKeyDown = (index, e) => {
      if (e.key === "Backspace") {
        if (!otp[index] && index > 0) requestAnimationFrame(() => inputRefs.current[index - 1]?.focus());
        else {
          const newOtp = [...otp];
          newOtp[index] = "";
          setOtp(newOtp);
    ...
    const handlePaste = (e) => {
      ...
      if (/^\d+$/.test(pastedData)) {
        const fullOtp = [...pastedData.split(''), ...Array(6 - pastedData.length).fill("")];
        setOtp(fullOtp);
        requestAnimationFrame(() => inputRefs.current[Math.min(pastedData.length, 5)]?.focus());
      }
    };
      ...
      try {
       const res = await axios.post(`${API_BASE_URL}/api/auth/verify-otp`,
        ...
        if (res.data.success) {
          ...
          // SuperAdmin created user → redirect to home
    ...
    return (
      <Box sx={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", p: 2 }}>
        <Box sx={{ p: 4, maxWidth: 400, width: "100%" }}>
          ...
          <Button fullWidth variant="outlined" onClick={handleResendOTP} disabled={isLoading || resendCooldown > 0}>
            {isLoading ? <CircularProgress size={24} /> : resendCooldown > 0 ? `Resend in ${resendCooldown}s` : "Resend Code"}
          </Button>
        </Box>
      </Box>
    );
