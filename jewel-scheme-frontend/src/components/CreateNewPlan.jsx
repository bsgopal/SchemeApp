...
const CreateNewPlan = ({ onCreatePlan }) => {
  ...
  useEffect(() => {
    if (id) {
      axios.get(`http://localhost:5000/api/scheme-groups/${id}`)
        .then(res => {
          if (res.data.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (id) {
        res = await axios.put(
          `http://localhost:5000/api/scheme-groups/${id}`,
          formData,
          { headers: { "Content-Type": "multipart/form-data" } }
        );
      } else {
        res = await axios.post(
          "http://localhost:5000/api/scheme-groups",
          formData,
          { headers: { "Content-Type": "multipart/form-data" } }
        );
      }
